...
<div class="container clearfix">
	...
	<div class="main-wrap">
		<div class="crumb-wrap">
			<div class="crumb-list"><i class="icon-font"></i><a href="<?php echo U('Index/index');?>">首页</a><span class="crumb-step">&gt;<span class="crumb-name"><a href="<?php echo U('furniture');?>">家居管理</a></span>&gt;<span class="crumb-name">家居编辑</span></span></div>
			</div>
		<form action="/index.php/Admin/Furniture/editOk" method="post" enctype="multipart/form-data">
			<div class="main">
				<p class="short-input ue-clear">
					<label>标题：</label>
					<input name="name" type="text" value="<?php echo ($data["name"]); ?>" placeholder="标题..." />
				</p>
				<div style="margin-left: 30px;margin-top: 15px;">
					<label>所属类别：</label>
					<div class="select-wrap" style="margin-top: 15px;margin-left: 85px;">
						<select name="cate_id">
							<?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($data["cate_id"] == $vo["id"] ): ?>selected='selected'<?php endif; ?> ><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</div>
				</div>
				<p class="short-input ue-clear">
					<label>附件：</label>
					<input name="file" type="file"/>(不修改请不要点击)
				</p>
				<p class="short-input ue-clear">
					<label>详细介绍：<script name="content" id="editor"  type="text/plain" style="width:888px;height:280px;">
						<?php echo (htmlspecialchars_decode(html_entity_decode($data["content"]))); ?>
					</script></label>
				</p>
			</div>
